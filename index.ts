...
function asyncTransform(csvFile: any): Promise<any> {
    ...
    return new Promise((resolve: any, reject: any) => csv({
        ...
        .fromFile(csvFile)
        .on("json", (result: { _key: string, [language: string]: string }) => {
            ...
            for (const lng of lngs) {
                ...
                if (m.has(lng)) {
                    const _t = m.get(lng);
                    Object.assign(_t[ns], { [key]: translated });
                } else {
                    m.set(lng, { [ns]: { [key]: translated } });
                }
            }
        })
        .on("done", () => {
...
downloadSheet()
    .then(asyncTransform)
    .then((result: Array<{ [language: string]: string }>) => fromPairs(result))
    .then(async (data: any) => {
